...
def load_file_all(file):
    ...
    for idx in range(0, len(lines), 13):
    ...
    f.close()
    return torch, onnx, rt_st, rt_dyn
...
def make_plot(title, file_name, results):
    ...
    plt.legend(['Torch', 'ONNX', 'RTNeural - xsimd', 'RTNeural - Eigen', 'RTNeural - STL'])

    plt.savefig(file_name)


torch_results, onnx_results, rt_stl_st, rt_stl_dyn = load_file_all('results/bench_stl.txt')
...
for k in base_results.keys():
    ...
    make_plot(f'{name} speed comparison (RTNeural compile-time)', f'plots/{k}_static.png',
        [torch_results[k], onnx_results[k], rt_xsimd_st[k], rt_eigen_st[k], rt_stl_st[k]])
